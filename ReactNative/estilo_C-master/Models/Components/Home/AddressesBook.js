...
import { View, Text, TouchableOpacity, Dimensions, StyleSheet, StatusBar, TextInput,Image, ScrollView,ImageBackground,FlatList, AsyncStorage } from 'react-native';
       ...
       try {
           ...
           if(this.state.addresses.length==0){
              if(this.props.Language=='AR'){
                 alert('لا يوجد عناوين')
              }else{
               alert('No Addresses Now')
              }
         }
   ...
   renderHeader() {
      return (
         <View style={[this.props.Language=='AR'? styles.row:styles.rowReversed, styles.shadow, { width: width, height: '10%', alignItems: 'center', backgroundColor: '#383B43', paddingHorizontal: 18 }]} >
           ...
           <Text style={{ flex:1,textAlign:'center',color: '#fff', fontSize: 17,fontFamily:'nexa_bold',}}>{this.props.Language=='AR'?'عناوين الحجـز':'Addresses Book'}</Text>
