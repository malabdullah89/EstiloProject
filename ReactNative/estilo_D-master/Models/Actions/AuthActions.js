...
export const UserRegister = (name , email, phone, password ,terms_of_user) => {
    return async (dispatch) => {
        dispatch({ type: 'REGISTER_ATTEMP' })
        NetInfo.fetch().then(state =>{
            if (state.isConnected){
        try {
            axios.post('http://ec2-3-19-72-49.us-east-2.compute.amazonaws.com/driver/sign_up/', {
                name,  email, phone, password,terms_of_user
            }).then((response)=> {
